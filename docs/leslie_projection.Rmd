...
# The line below creates a time series of 1-8 years by 1 to estimate population size
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below makes a matrix with the column length fish_year0 and row length of time_yr
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below...
proj_year[1, ] <- fish_year0

# estimate the population size  using the previous year. multiplies fish_leslie x the previous year
...
#updates col names for population stages
colnames(proj_year) <- c("eggs", "fry", "juvenile", "adult")

# creates a new df with time_yr, proj_year and changed names_to = "lifestage", values_to = "stage_population"
...
# used the proj_df data.frame to plot the stage population at each year
